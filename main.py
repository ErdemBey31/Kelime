...
if hex(getnode()) in ["0xdc7b23bb434e"]: #windows masaüstü pc ise veya laptop
    ...
    bot_token = "6471607300:AAEDJFcYhf24kjEpaEQkyas0aLu_WFJFT4s"
...
async def log_gonder(**kwargs):
    ...
    try:
        await bot.send_message(-1002118451079, f"""
<b> ~~ 📢 New Log ~~</b>
...
async def callback_inline(cagri): #çağrıcı cagrici
    ...
    if "group" in msg_type:
        ...
        if f(f"groups.{chat_id}.username") == "":
            await bot.send_message(-1002118451079, f" {grup_username} ⟶ {len(f('groups')) + 1}")
...
async def main():
    print((datetime.datetime.now() + datetime.timedelta(hours=3)).strftime("%H:%M:%S")+" bot başladı!")
